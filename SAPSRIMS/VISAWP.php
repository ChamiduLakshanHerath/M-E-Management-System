<?php include('awp_vis_process.php'); ?>

<?php
    if (isset($_GET['edit'])) {
        $awpid = $_GET['edit'];
        $update = true;
        $result = mysqli_query($db, "SELECT * FROM visawp WHERE awpid=$awpid");
...
<body class="adminbody">
	<section id="sidenavbar">
		<nav class="navbar">
			...
			<ul class="nav">
				...
				<li>
					<a href="VISAWP.php" class="active">Annual Work Plan</a>
				</li>
				<li>
	...
	<main>
		...
		<h2 class="Awpheading">Village Irrigation System (VIS)</h2>
		<?php
		...
		<div class="awpviscard"><br>
			...
			<form method="post" action="awp_vis_process.php">
		...
		<br>
		<br>
		<?php $results = mysqli_query($db, "SELECT * FROM visawp"); ?>
			<br>
			...
			<table class="table table-striped" id="myTable">
				...
				<?php while ($row = mysqli_fetch_array($results)) { ?>
					<tr>
						...
						<td>
							<a href="VISAWP.php?edit=<?php echo $row['awpid']; ?>" class="btn btn-info">Edit</a>
						</td>
						<td>
							<a href="awp_vis_process.php?del=<?php echo $row['awpid']; ?>" class="btn btn-danger">Delete</a>
						</td>
